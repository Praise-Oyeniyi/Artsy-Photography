...
const BidSection = ({bidFunction, setBid, addBid, bidss,bid}) => {
  return (
    ...
    <div className="main-auction-r-inner h-full">
        <div className="bids space-y-5 h-[85%] overflow-y-auto scrollbar-hide" >
            {bidss.map(e=>(
                <div className="profile flex-center justify-start gap-x-5" key={e.id}>
                    <img src={e.image} alt="profile-rep" className='h-full'/>

                    <div className="h-full profile-bid flex-column space-y-1 text-[#333] text-24">
        ...
        <div className="form sticky w-full h-auto flex-column justify-center space-y-1 mt-1">
            ...
            <div className="inputs flex-center">
                <div className='border-grey border w-5/6 h-10 rounded-2xl p-2 px-4 flex-center'>
                    <input 
                        type="text"  
                        className='w-full h-full  outline-none border-none placeholder-auc-creator-text text-15' 

...
const Featured = ({next, useNavigate, FtData}) => {
  ...
  return (
    <div className="featured w-full pb-20">

        <div className="featured-inner w-5/6 mx-auto space-y-10">
          <h3 className="featured-header pb-6 featured-border text-48 font-medium">
            Featured Products
          </h3>
          ...
          <div className="featured-product-outer space-y-10">
            {FtData.map((e)=>(
              <div className={`featured-product w-full flex space-x-10 featured-border ${e.id===3?'!border-b-0':''}`} key={e.id}>
                <div className={`featured-product-img w-full ${e.id%2===0? 'order-2 ml-10':''}`} onClick={()=>navigate(`/market/${e.id}`)}>
                ...
                <div className={`featured-product-text ${e.id%2===0? 'order-1':''}`}>
                  <h3 className='text-40 nav-logo font-bold'>{e.name}</h3>
                  <p className='text-24'>{e.description}</p>
                  <div className='flex-center'>

                    <div className="flex-center justify-start space-x-8">
                      <div className="w-2/6 images relative h-12">
                      ...
                      <h4 className='text-24 font-semibold w-auto'>64 major creators</h4>
                    </div>

                    <img src={next} alt="marketplace-icon" className='border rounded-full p-3 h-14 w-14' onClick={()=>navigate(`/market/${e.id}`)}/>
                  </div>

...
const FilterTop = ({filterAll, searched, active, setActive}) => {
  ...
  return (
    <div className='my-14 mt-24 flex-center w-full sm:my-10 sm:block sm:space-y-3 sm:mb-0'>
        ...
        <h4 onClick={()=>navigate('/')} className="text-lg font-semibold sm:mx-3 text-[#BCB7B7] lg:hidden">Home / Marketplace /<span className='text-black font-semibold'> Editorials</span></h4>
        
        <p className='lg:hidden sm:mx-3 text-[#BCB7B7] text-base italic'>{`Showing ${filterAll.length >=1? filterAll.length : '9'} of 9 results`}</p>
        ...
        <div className='counter-box w-[80%] h-14 flex-center px-4 py-2 sm:py-0 bg-white rounded-lg text-24 sm:w-full'>
            ...
            <div className='lg:hidden flex-center justify-start space-x-2' onClick={()=>setActive(!active)}>
              <h4 className='text-lg'>Filters</h4>
              <i className="fa-solid fa-chevron-down text-lg"></i>
            </div>
            <p className='sm:hidden'>{`See ${filterAll.length >=1? filterAll.length : '9'} of 9 results`}</p>
